...
db-start:
	podman-compose up -d mortgage-ai-db

db-stop:
	podman-compose down mortgage-ai-db

db-logs:
	podman-compose logs -f mortgage-ai-db
...
build-images:
	...
	@podman build -f packages/api/Containerfile -t mortgage-ai-api:$(IMAGE_TAG) .
	@echo "Building UI image..."
	@podman build -f packages/ui/Containerfile -t mortgage-ai-ui:$(IMAGE_TAG) .
	@echo "✅ Images built successfully"

push-images:
	@NEED_BUILD=false && \
	if ! podman image exists mortgage-ai-api:$(IMAGE_TAG); then echo "API image not found"; NEED_BUILD=true; fi && \
	if ! podman image exists mortgage-ai-ui:$(IMAGE_TAG); then echo "UI image not found"; NEED_BUILD=true; fi && \
	...
	podman tag mortgage-ai-api:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/mortgage-ai-api:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/mortgage-ai-api:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push API image. Common solutions:"; echo "  1. Ensure you have push permissions: oc policy add-role-to-user system:image-builder $$(oc whoami) -n $(NAMESPACE)"; echo "  2. Check if namespace exists: oc get project $(NAMESPACE)"; echo "  3. Try creating the project: oc new-project $(NAMESPACE)"; exit 1; fi && \
	echo "Tagging and pushing UI image..." && \
	podman tag mortgage-ai-ui:$(IMAGE_TAG) $$REGISTRY_URL/$(NAMESPACE)/mortgage-ai-ui:$(IMAGE_TAG) && \
	if ! podman push $$REGISTRY_URL/$(NAMESPACE)/mortgage-ai-ui:$(IMAGE_TAG); then echo ""; echo "❌ Failed to push UI image. Common solutions:"; echo "  1. Ensure you have push permissions: oc policy add-role-to-user system:image-builder $$(oc whoami) -n $(NAMESPACE)"; echo "  2. Check if namespace exists: oc get project $(NAMESPACE)"; echo "  3. Try creating the project: oc new-project $(NAMESPACE)"; exit 1; fi && \
